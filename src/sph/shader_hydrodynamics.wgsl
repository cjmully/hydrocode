...
@group(2) @binding(0)
var<storage, read_write> spatial: array<SpatialLookup>;

@group(2) @binding(1)
var<storage, read_write> start_index: array<u32>;

@group(2) @binding(2)
var<storage, read> grid_param: GridParam;

@group(3) @binding(0)
var<uniform> coord_offset: array<vec3i, 27>;
...
fn density_interpolant(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var offset_idx: u32 = u32(0);
    for (offset_idx = 0u; offset_idx < 9u; offset_idx ++) {
        // Calculate hash key
        let grid_coord = particle.coord + coord_offset[offset_idx];
        ...
        let idx0 = start_index[key];
        var spatial_idx: u32 = u32(0);
        for (spatial_idx = idx0; spatial_idx < array_length; spatial_idx++) {
            ...
            let rvec_ab = (vec3f(particle.coord - neighbor.coord) + particle.position - neighbor.position) * grid_param.grid_size;
